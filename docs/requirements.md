...
- 過去の記録を編集・削除可能
- 「履歴」画面から編集を開始
- ラジオボタンで該当レコードを選択し、「編集」ボタンで編集モードに移行
- 編集は新規記録と同様のフォームを使用
- 「保存」ボタンで編集内容を保存
- モーダル右上の「削除」ボタンで記録を削除（確認ダイアログあり）

### 3.2 睡眠履歴表示機能

#### 3.2.1 過去記録の一覧表示（履歴画面）
- 「履歴」画面を用意
- 過去の記録をテーブル形式で一覧表示
- 表示項目：ラジオボタン、日時、睡眠時間、睡眠の種類（夜/昼）
- 日付ごとにグループ化して表示
- ラジオボタンで記録を選択し、ヘッダーの「編集」ボタンで編集モードに移行
...
#### 4.2.1 メイン画面（index.html）
- **ヘッダー**
  - アプリ名「RecSleep」を左側に配置
  - 「履歴」リンクを右側に配置
- **週次/月次グラフ**
  - ヘッダー直下に配置
  - 週次/月次の切り替えUIをグラフ上部に配置
  - 積層グラフを表示（日付をまたぐ睡眠は分割表示）
- **入力フォーム**
  - グラフの下に配置
  - グループ化されたレイアウト
  - 睡眠開始日時（デフォルト：前日22:00）
  - 睡眠時間
  - 記録するボタン

#### 4.2.2 履歴画面（history.html）
- **ヘッダー**
  - 「戻る」ボタン（左側）
  - タイトル「履歴」（中央）
  - 「CSV出力」「編集」ボタン（右側）
- **過去記録一覧**
  - テーブル形式で表示
  - 日付ごとにグループ化
  - 各行：ラジオボタン | 日時 | 睡眠時間 | 種別
  - ラジオボタンで選択し、ヘッダーの「編集」ボタンで編集
- **編集モーダル**
  - 記録の編集・削除が可能
  - 右上に小さな削除ボタン
  - キャンセル・保存ボタン
